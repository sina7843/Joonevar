...
| `/declaration`, `/declaration/new`, `/declaration/[id]` | PROTO | `242:6517` مسیر شخصی | s20 | — | A-015 — پیاده‌شده در PROMPT-017؛ بدون پرداخت، بدون ذخیره متن/فایل/امضا و بدون هیچ اثر رسمی |
...
| `/assoc/postal` | فهرست درخواست‌های پستی، فقط ثبت درخواست | s14 | D17 | — پیاده‌شده در PROMPT-018 |
...
| `/admin/breeds` (به‌جای `/admin/settings/reference-data`) | فهرست مرجع نژادها به‌عنوان داده مدیریت‌شده؛ کنارگذاشتن به‌جای حذف | s21 | D16 | — پیاده‌شده در PROMPT-018 (DEC-0113) |
| `/admin/settings/history` | تاریخچه تغییر تنظیمات با Actor و مقدار قبلی | s21, s23 | D16 | A-030 |
| `/admin/audit` | مرور Audit در حد مجوز، فقط‌خواندنی با فیلتر نوع پرونده | s23 | — | A-026 — پیاده‌شده در PROMPT-018 |
