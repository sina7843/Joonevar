...
| خانه و خدمات (۰۱۳) | `src/services/catalogue.ts`، `src/services/service.ts`، `app/(public)/services`، `app/(public)/page.tsx` | کاتالوگ خدمات متن ثابت کد است و جدول تازه ندارد؛ هر هزینه فقط کلید `product_setting` است و ثبت‌نشده یعنی «تعیین‌نشده»، نه رایگان؛ هیچ زمان یا نوبتی وعده داده نمی‌شود؛ CTA فقط به مسیر محافظت‌شده موجود فاز یک می‌رود؛ `/services` بخش §۴ نیست ولی در `SITEMAP_SECTIONS` می‌آید (DEC-0173) |
| استعلام اصالت (۰۱۴) | `src/verification/model.ts`، `src/verification/service.ts`، `app/(public)/verify`؛ جدول تازه `verification_attempt` و کلید `verification.attempt_hourly_limit` | پاسخ از خودِ `registration_sheet`/`pedigree`/`puppy_card` خوانده می‌شود و نسخه عمومی موازی ساخته نمی‌شود؛ «باطل» چون در فاز یک منبع ندارد ساخته نشد و `REPLACED` فقط از `correction_notice_fa` می‌آید؛ داده عمومی حداقلی بدون مالک، میکروچیپ و نام حیوان؛ تلاش پرتکرار با نشانی هش‌شده و سقف مدیریت‌شده محدود می‌شود؛ صفحه `noindex` است (DEC-0174) |
| جغرافیا و صفحات محلی (۰۱۵) | `src/geo/model.ts`، `src/geo/service.ts`، `src/ui/map.tsx`، `app/(public)/places`؛ ستون `city.slug` و تنظیم `integration.map.embed_url_template` | slug شهر از نام فارسی ساخته می‌شود (نام لاتین اختراع نمی‌شود) و استان `code` لاتین خود را نگه می‌دارد؛ صفحه مکانِ بدون رکورد منتشرشده `noindex` است و در sitemap نمی‌آید؛ شمارش فقط از رکورد منتشرشده با محل عمومی؛ نقشه فقط با قالب embed ثبت‌شده توسط اپراتور رندر می‌شود و محل غیرعمومی هیچ چیزی رندر نمی‌کند (DEC-0175) |
| پنل مدیریت (۰۱۶) | `src/admin/queues.ts`، `src/admin/merge-model.ts`، `src/admin/merge.ts`، `app/admin/merge`، تابلوی صف در `app/admin/page.tsx`؛ ستون‌های `merged_into_*_id` | صف تازه‌ای ساخته نمی‌شود: تابلو از همان توابع صفحه‌های صف می‌پرسد و صفی که اکتور اجازه‌اش را ندارد نمایش داده نمی‌شود (صفر نشان داده نمی‌شود)؛ ادغام رکورد را حذف نمی‌کند و نشانی عمومی‌اش به رکورد اصلی اشاره می‌کند؛ هدایت فقط یک گام است؛ رکورد ادغام‌شده از فهرست‌ها بیرون می‌رود ولی نشانی خودش پاسخ می‌دهد (DEC-0176) |
| حریم خصوصی و سوءاستفاده (۰۱۷) | `src/privacy/limits.ts`، `src/privacy/policy.ts`؛ سقف صفحه در پنج صف عملیاتی | بندهای §۲۰ به‌صورت داده با مسیر کدِ اجراکننده نوشته شده‌اند و تست تأیید می‌کند هر بند صاحب دارد؛ حساب پنجره و سقف یک‌جاست و پیام رد هیچ شمارشی نمی‌دهد؛ صف‌های عملیاتی که صفحه خود را دستی می‌ساختند حالا از سقف مشترک `MAX_PAGE_SIZE` می‌گذرند و عدد تازه‌ای اختراع نشد؛ جریان کد ورود به‌عنوان پروتکل دست‌نخورده ماند (DEC-0177) |
| کارایی و دسترس‌پذیری (۰۱۸) | `src/a11y/contrast.ts`، `tests/domain/a11y.test.ts`، `tests/browser/a11y.test.ts`؛ `app/media/[id]/route.ts`، `src/content/public-views.tsx` | کنتراست با فرمول WCAG روی توکن‌های واقعی حساب می‌شود و تست تأیید می‌کند همان مقدارهای `globals.css` است؛ رندر per-request صفحه‌های عمومی عمداً می‌ماند چون هدر وضعیت حساب را نشان می‌دهد؛ تصویر محتوا با ETag (همان sha256 موجود) بازدید دوم را ۳۰۴ می‌کند و `must-revalidate` برای Moderation سر جایش است؛ فضای تصویر با aspect-ratio رزرو می‌شود چون ابعاد ذخیره نمی‌شود؛ پوشش RTL و focus از `rtl.test.ts` تکرار نشد (DEC-0178) |
| مهاجرت و تست یکپارچه (۰۱۹) | `src/db/schema/taxonomy.ts`، `src/db/seed/taxonomy.ts`، مهاجرت `0029_taxonomy-seed.sql`؛ `tests/domain/migrations.test.ts`، `tests/db/taxonomy.test.ts`، توسعه `tests/db/migrations.test.ts`؛ `docs/qa/phase-2-acceptance.md` | برگشت‌پذیری با فایل `down` ادعا نشد: ثابتِ «هیچ مهاجرتی حذف یا تغییرنام نمی‌کند» روی متن SQL تست می‌شود و `SET NOT NULL` فقط پس از پرکردن ستون مجاز است؛ هشت Taxonomy نسخه‌دار شدند و Seed فقط ورودی غایب را اضافه می‌کند و نام تغییریافته یا ورودی خاموش‌شده اپراتور را بازنمی‌گرداند؛ کاتالوگ عیناً همان داده مهاجرت‌هاست و تست آن را با پایگاه‌داده مقایسه می‌کند؛ جدول‌های فاز ۲ و پیوند کلید خارجی به `reference_breed`/`vet_profile`/`vet_location`/`account` قفل شد (P2-D15)؛ ماتریس کامل دو تست شکننده را آشکار کرد (ناوبری منجمد ادمین، انتخاب دامپزشک بر اساس جایگاه) که نقص محصول جا می‌زدند (DEC-0179) |
| آمادگی انتشار و تحویل (۰۲۰) | `tests/browser/release.test.ts`؛ `src/health/service.ts` (نسل Taxonomy)؛ `docs/ops/release-readiness-phase-2.md`، بخش ۹ `docs/ops/runbook.md`، بخش ۸ `docs/ops/operator-guide.md` | سلامتِ لینک با **دنبال‌کردن** نشانی‌های واقعی sitemap اثبات می‌شود، نه با فهرست بخش‌ها؛ هیچ نشانی محافظت‌شده‌ای نباید منتشر شود؛ `/api/health` وضعیت `degraded` را با نام‌بردن کلیدهای تعیین‌نشده و نسل هر Taxonomy می‌دهد و تست همان را قفل می‌کند؛ اسناد فاز یک بازنویسی نشدند و فاز دو سند تحویل خودش را گرفت؛ ناپایداری آپلود تصویر محتوا به‌جای پنهان‌شدن، با شواهد هر چهار اجرا ثبت شد (DEC-0180) |
