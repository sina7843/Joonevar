/**
 * Two-party allocation and Puppy Cards in a real browser — gate
 * `puppy-card-browser`.
 *
 * Two owners of one issued permit walk §19.3 and §19.4 through the screens: a
 * proposal, a rejection, a revised proposal, two confirmations, and only then a
 * paid card per puppy. Fixtures are SYNTHETIC and on the reserved 0999 range.
 */
...
const SHOTS = path.join('docs', 'reports', 'screenshots', 'prompt-016');
...
const LOCATION_NAME = 'SYNTHETIC کلینیک کارت توله ' + RUN;
...
let chipCounter = 0;
const nextChip = () => '9' + RUN.padStart(6, '0') + String(8_000_000 + (chipCounter += 1)).padStart(8, '0');
...
interface Fixture {
  readonly sire: { context: BrowserContext; page: Page };
  readonly dam: { context: BrowserContext; page: Page };
  readonly permitId: string;
  readonly litterId: string;
  readonly puppyCodes: readonly string[];
}
...
before(async () => {
  ...
  // The card tariff starts unset, so a test can prove that state.
  await setSetting('fee.puppy_card_toman', '');
  ...
  try {
    ...
    const male = await pedigreedAnimal(sire.page, vetPage, centrePage, 'سگ نر کارت', 'MALE');
    const dam = await newOwner('مالک ماده آزمایشی');
    const female = await pedigreedAnimal(dam.page, vetPage, centrePage, 'سگ ماده کارت', 'FEMALE');
    ...
    // Two puppies born alive, recorded through the birth screen itself.
    await sire.page.goto(BASE_URL + '/mating/permits/' + permitId + '/birth', { waitUntil: 'load' });
    await sire.page.getByTestId('born-on').fill(day(-7));
    ...
    const litterId = new URL(sire.page.url()).pathname.split('/')[2]!;

    fixture = {
      sire: { context: sire.context, page: sire.page },
      dam: { context: dam.context, page: dam.page },
      permitId,
      litterId,
      puppyCodes: codes,
    };
...
test('a proposal is not agreement: one approval leaves it pending and the card locked', async () => {
  const sire = fixture.sire.page;
  const [first, second] = fixture.puppyCodes as [string, string];

  await sire.goto(BASE_URL + '/litters/' + fixture.litterId + '/allocation', { waitUntil: 'load' });
  // The pre-birth rule is shown as context and assigns nothing by itself.
  await expectText(sire, 'توافق پیش از تولد (فقط زمینه پیشنهاد)');
  await expectText(sire, 'مالکیت هیچ توله مشخصی را نهایی نمی‌کند');
  await expectText(sire, 'داوری نمی‌کند');
  await sire.screenshot({ path: path.join(SHOTS, 'allocation-empty.png'), fullPage: true });

  await sire.getByTestId('assign-' + first).selectOption({ index: 1 });
  await sire.getByTestId('assign-' + second).selectOption({ index: 1 });
  await sire.getByTestId('submit-allocation').click();
  await sire.getByTestId('allocation-items').waitFor();
  assert.equal(await sire.getByTestId('allocation-status').innerText(), 'در انتظار تأیید هر دو طرف');

  // The proposer approves; that is one voice, not agreement.
  await sire.getByTestId('allocation-decision-APPROVE').check();
  await sire.getByTestId('submit-allocation-response').click();
  await sire.getByTestId('pending-both-note').waitFor();
  assert.equal(await sire.getByTestId('allocation-status').innerText(), 'در انتظار تأیید هر دو طرف');
  await sire.screenshot({ path: path.join(SHOTS, 'allocation-pending.png'), fullPage: true });

  // The card is locked, says exactly why, and opens no payment path.
  await sire.goto(BASE_URL + '/puppy-cards/checkout?permit=' + fixture.permitId, { waitUntil: 'load' });
  await expectText(sire, 'تا نهایی‌شدن تخصیص با تأیید هر دو طرف، کارت این توله قفل است');
  assert.equal(await sire.getByTestId('pay-cards').count(), 0);
  await sire.screenshot({ path: path.join(SHOTS, 'card-locked.png'), fullPage: true });
});

test('a rejection needs a new version, and both confirmations finalise it', async () => {
  const sire = fixture.sire.page;
  const dam = fixture.dam.page;
  const [first, second] = fixture.puppyCodes as [string, string];

  // The other side rejects the proposal with a reason.
  await dam.goto(BASE_URL + '/litters/' + fixture.litterId + '/allocation', { waitUntil: 'load' });
  await dam.getByTestId('allocation-decision-REJECT').check();
  await dam.getByTestId('allocation-reject-reason').fill('با این تقسیم موافق نیستم.');
  await dam.getByTestId('submit-allocation-response').click();
  await dam.waitForFunction(
    () =>
      document.querySelector('[data-testid="allocation-status"]')?.textContent?.includes('ردشده'),
    undefined,
    { timeout: 20_000 },
  );
  assert.equal(await dam.getByTestId('allocation-status').innerText(), 'ردشده؛ نیازمند نسخه جدید');
  await dam.screenshot({ path: path.join(SHOTS, 'allocation-rejected.png'), fullPage: true });

  // A revised proposal is version 2, and the earlier version stays readable.
  await sire.goto(BASE_URL + '/litters/' + fixture.litterId + '/allocation', { waitUntil: 'load' });
  await sire.getByTestId('assign-' + first).selectOption({ index: 1 });
  await sire.getByTestId('assign-' + second).selectOption({ index: 2 });
  await sire.getByTestId('allocation-note').fill('نسخه اصلاح‌شده پس از توافق بیرون از سامانه.');
  await sire.getByTestId('submit-allocation').click();
  await sire.getByTestId('allocation-version-2').waitFor();
  await expectText(sire, 'ردشده؛ نیازمند نسخه جدید');

  // Both sides confirm the same version, and only that finalises it.
  await sire.getByTestId('allocation-decision-APPROVE').check();
  await sire.getByTestId('submit-allocation-response').click();
  await sire.getByTestId('pending-both-note').waitFor();

  await dam.goto(BASE_URL + '/litters/' + fixture.litterId + '/allocation', { waitUntil: 'load' });
  await dam.getByTestId('allocation-decision-APPROVE').check();
  await dam.getByTestId('submit-allocation-response').click();
  await dam.waitForFunction(
    () =>
  ...
  assert.equal(await dam.getByTestId('allocation-status').innerText(), 'نهایی‌شده با تأیید دوطرفه');
  await dam.screenshot({ path: path.join(SHOTS, 'allocation-final.png'), fullPage: true });
});

test('cards are paid once, issued per puppy, and are not a registration sheet', async () => {
  const sire = fixture.sire.page;
  const dam = fixture.dam.page;
  const [first, second] = fixture.puppyCodes as [string, string];

  // Until the tariff exists, no payment path opens and no amount is assumed.
  await sire.goto(BASE_URL + '/puppy-cards/checkout?permit=' + fixture.permitId, { waitUntil: 'load' });
  await expectText(sire, 'تعرفه کارت توله هنوز ثبت نشده است');
  assert.equal(await sire.getByTestId('pay-cards').count(), 0);
  await sire.screenshot({ path: path.join(SHOTS, 'card-fee-not-configured.png'), fullPage: true });
  await setSetting('fee.puppy_card_toman', CARD_FEE);

  await sire.reload({ waitUntil: 'load' });
  await expectText(sire, CARD_FEE_FA);
  await expectText(sire, 'با برگه ثبتی، شجره‌نامه یا نتیجه ژنتیک یکی نیست');
  // The second puppy belongs to the other party in the final version.
  assert.equal(
    await sire.getByTestId('card-reason-' + second).innerText(),
    'مالک نهایی این توله طرف دیگر پرونده است.',
  );

  await sire.getByTestId('card-pick-' + first).check();
  await sire.getByTestId('pay-cards').click();
  await sire.waitForURL('**/dev/gateway**');
  await sire.getByTestId('gateway-pay').click();
  await sire.waitForURL('**/puppy-cards/return**');
  await expectText(sire, 'پرداخت تأیید شد');
  await sire.screenshot({ path: path.join(SHOTS, 'card-paid.png'), fullPage: true });

  // The card exists as its own document, before any registration sheet.
  await sire.goto(BASE_URL + '/puppy-cards/checkout?permit=' + fixture.permitId, { waitUntil: 'load' });
  await sire.getByTestId('issued-card-' + first).click();
  await sire.waitForURL((url) => url.pathname.startsWith('/documents/puppy-card/'));
  const cardUrl = sire.url();
  assert.match(await sire.getByTestId('card-no').innerText(), /PC-[A-Z0-9]{8}/);
  assert.equal(await sire.getByTestId('card-allocation-version').innerText(), '2');
  await expectText(sire, 'با برگه ثبتی، شجره‌نامه یا نتیجه ژنتیک یکی نیست');
  await sire.screenshot({ path: path.join(SHOTS, 'card-document.png'), fullPage: true });

  // The other owner cannot open a card that is not theirs, and issues their own.
  await dam.goto(cardUrl, { waitUntil: 'load' });
  await expectText(dam, 'پیدا نشد');
  await dam.goto(BASE_URL + '/puppy-cards/checkout?permit=' + fixture.permitId, { waitUntil: 'load' });
  await dam.getByTestId('card-pick-' + second).check();
  await dam.getByTestId('pay-cards').click();
  await dam.waitForURL('**/dev/gateway**');
  await dam.getByTestId('gateway-pay').click();
  await dam.waitForURL('**/puppy-cards/return**');
  await expectText(dam, 'پرداخت تأیید شد');
  await dam.goto(BASE_URL + '/puppy-cards/checkout', { waitUntil: 'load' });
  assert.equal(await dam.locator('[data-testid="my-cards"] > li').count(), 1, 'each owner sees only their own');
  await dam.screenshot({ path: path.join(SHOTS, 'my-cards.png'), fullPage: true });
});
